...
<!-- 見送り追認・受入シナリオなどで人間の判断に回した Issue があればここに列挙する(番号 + 1行説明)。
     1件も無い場合はその旨を明記する: 「判断してほしいことはありません(マージゲートを満たせば自動でマージされます)」 -->
...
## レビュー結果(見送った suggestion)

<!-- レビュー suggestion のうち、見送り基準 (a)(独立した PR が必要な別リファクタリング相当の規模)で見送ったものをここに記録する(指摘の要約と見送った理由を各1〜2行)。
     基準 (b)(ユーザーの意思決定が必要な設計判断)の見送りはここではなく `[判断待ち]` Issue に切り出し、「あなたに判断してほしいこと」節からリンクする。
     見送った suggestion が1件も無ければこの節ごと削除する。 -->
