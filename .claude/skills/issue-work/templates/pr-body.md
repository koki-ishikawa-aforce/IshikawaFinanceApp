...
<!-- 各レビュー(/ddd-review 等)の suggestion のうち、見送り基準 (a)(独立した PR が必要な別リファクタリング相当の規模)に該当し
     その場で修正しなかったものを、指摘内容と見送り理由のセットで列挙する(1件につき1〜2行)。
     見送り基準 (b)(設計判断)に該当するものは、代わりに `[判断待ち]` Issue を起票し「あなたに判断してほしいこと」節にリンクする(ここには書かない)。
     見送った suggestion が1件も無い場合はその旨を明記する: 「見送った suggestion はありません」 -->
