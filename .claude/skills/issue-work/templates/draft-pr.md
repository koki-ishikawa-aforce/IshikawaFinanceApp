# Draft PR 本文テンプレート(無人モード用)

無人モードで Draft PR を作成するときの本文フォーマット。`.github/PULL_REQUEST_TEMPLATE.md` の必須項目(検証チェックリスト・Closes)を包含している。SKILL.md の「人間向け報告の執筆ルール」を必ず適用する。以下の `<!-- -->` は書き方の指示であり、出力には含めない。
