...
#### 3e. 決定が要件分解を必要とする場合(同一セッションでの継続)

決定の帰結が「別途 `/issue-create` で要件を分解する」となる場合(例: `[改善案]` で方針 A/B を採用したが、実装 Issue に落とすには設計の対話が要る場合)、決定の記録後に AskUserQuestion で「**このセッションで続けて分解する / 後で行う**」を確認する。

- **続ける**: アジェンダの残件をすべて消化し終えたあと(手順6の最終報告の前後どちらでもよい)、同一セッション内で `/issue-create` を起動して分解まで行う。決定の文脈が揃っているうちに分解まで閉じることで、改善ループの滞留(`docs/workflow/04-principles.md` 未解決の課題)を1ホップ減らせる
- **後で**(既定): 従来どおり決定コメントに「別途 `/issue-create` で分解する」と記録して次のアイテムへ進む

この継続は opt-in であり、全件で必須にしない(`/decide` の1件あたりを軽く保ち、キュー消化を優先する設計を崩さないため)。分解を始めるのは必ず**アジェンダの消化後**とし、消化ループの途中に `/issue-create` の対話を挟まない。
