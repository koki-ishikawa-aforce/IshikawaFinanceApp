...
- 対象コンテキストを明記する
- **着手をブロックする先行 Issue は「依存(ブロッカー)」節に `#N` で列挙する**(その Issue がクローズされるまで着手できないもの。無ければ「なし」)。単に関連する・参照するだけの Issue は着手をブロックしないので「参考資料」へ書く(無人モードの候補選定は「依存(ブロッカー)」「先行」節のみを着手ブロック条件として見る)
...
- 「依存(ブロッカー)」「先行」節の Issue が open でも他の基準を満たすなら付与してよい(Routine の依存チェックが先行 Issue のクローズまで着手を自動で遅延する。「関連」「参考資料」の Issue 参照は依存ではなく着手をブロックしない)。先行 Issue の実装結果次第で受け入れ条件が変わりうる場合のみ付与せず、「#X のマージ後に `/backlog-ready` で再判定」と報告する
