...
3. **依存の扱い**: 着手をブロックする先行 Issue は本文の「依存(ブロッカー)」「先行」節に書く(「関連」「参考資料」に挙げた Issue 参照は依存ではなく、着手をブロックしない)。「依存(ブロッカー)」「先行」に挙げられた Issue が open でも、他の基準を満たすなら付与してよい(着手は Routine 側の依存チェックが先行 Issue のクローズまで自動で遅延する)。ただし先行 Issue の実装結果次第で本 Issue の受け入れ条件やアプローチが変わりうる場合は付与せず、マージ後に再判定する
