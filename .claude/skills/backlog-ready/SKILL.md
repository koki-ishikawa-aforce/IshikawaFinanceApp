...
3. **依存の扱い**: 本文の「依存」「先行」節に挙げられた Issue が open でも、他の基準を満たすなら付与してよい(着手は Routine 側の依存チェックが先行 Issue のクローズまで自動で遅延する)。「関連」「参考資料」内の Issue 参照は着手ブロック条件にならない(単なる参照)。ただし先行 Issue の実装結果次第で本 Issue の受け入れ条件やアプローチが変わりうる場合は付与せず、マージ後に再判定する
