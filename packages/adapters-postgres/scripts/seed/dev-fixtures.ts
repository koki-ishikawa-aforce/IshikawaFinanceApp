/**
 * 開発用フィクスチャの seed（本番には投入しない）
 *
 * 夫婦2人のダミーユーザー・口座・カード未払金・取引を投入して、
 * ローカルでダッシュボードやレポートの見た目を確認できるようにする。
 * 本番 DB に入れてはいけないデータのため、規定マスタ（masters.ts）とは
 * 明確に分離し、CLI の --with-dev-fixtures 指定時のみ実行される。
 *
...
export const DARLING_ID = 'U_DARLING_DEV'
