/**
 * SpouseCompletionQuery の PostgreSQL 実装
...
export interface PostgresSpouseCompletionQueryDeps {
  /** 配偶者行が未登録のとき spouseUserId を補う（PostgresAllowlistQuery.fetch を渡す） */
  fetchAllowlist: () => Promise<Allowlist>
  /** awaiting_spouse.detectedAt に使う現在時刻（テスト決定性のため注入） */
  now: () => Date
}

export class PostgresSpouseCompletionQuery implements SpouseCompletionQuery {
  constructor(
    private readonly db: Db,
    private readonly deps: PostgresSpouseCompletionQueryDeps,
  ) {}
