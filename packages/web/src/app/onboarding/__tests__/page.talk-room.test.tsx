 ...
 * 検知を待っている旨の案内で待機し、次のステップへは進めないこと、「最新の状態を確認」を
...
describe('共通トークルームへの参加の確認', () => {
  it('未検知のあいだは検知を待っている旨の案内が出て、自己申告で進む手段は無い（#298）', async () => {
    renderPage()

    expect(await screen.findByText(/参加の検知を待っています。参加していれば/)).toBeInTheDocument()
