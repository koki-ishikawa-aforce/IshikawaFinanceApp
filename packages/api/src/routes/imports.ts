...
export function importsRoutes(deps: ImportsRoutesDeps): Hono<AppEnv> {
   ...
   * これまでどおり行う。直近の実行が失敗（終端）で終わっている場合は、この間隔の代わりに
   * 短い `FAILED_MANUAL_MAIL_IMPORT_RETRY_FLOOR_MS`（既定 30 秒）を適用する — もう動いて
   * いないため二重起動を防ぐ狙いは当てはまらないが、外部 API（Gmail）のレート制限中の連打を
   * 防ぐ下限は残す（#628）。日次の自動起動はこの経路を通らないため制限を受けない。
   */
