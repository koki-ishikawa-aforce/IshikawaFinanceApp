 ...
 * 防御は 2 段構えになっている。
 *
 * 1. **既存の記録は上書きしない**。参加先の変更（招待し直し）を検知する経路は現状ない
 *    （自己申告 API は #298 で廃止済み。回復は招待のやり直しのみ。ギャップは #590 で追跡）
 ...
 * 記録してしまうと 1 の上書き禁止によって誤った配信先が固定され、直す手段が無くなるため、
 * 確証が無いときは見送る側へ倒す。`join` は招待の瞬間にしか発生せず再送されないので、
 * 見送った回の回復は招待のやり直しによる。
 */
