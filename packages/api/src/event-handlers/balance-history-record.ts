 ...
 *
 * 例外は NISA 積立累計の補正（#458）で、ここだけは**手で回復できる**。補正は差し替えなので
 * 同じ累計で `PUT /api/accounts/:id/contribution` を叩き直しても累計は二重に動かず、新しい
 * イベントIDで正しい値の点が積まれる。復元に要る値と日時は口座の `contribution.manualEntries`
 * に残っている。逆に `nisa_contribution` 軸は「次に同じ軸が動けば線が復帰する」があてにできない
 * （振込由来の自動加算 NisaContributionAdded を発行する本番経路が #390 待ちで未接続のため、
 * この補正と初期残高の修正以外にこの軸へ点を積むものが無い）。
 */
import {
