...
- 集約: `Transaction`（`unclassified` / `classified` / `deleted`。未分類→分類 `classify` / 分類済み生成 `createClassifiedTransaction` / 削除 `deleteTransaction` を含む）, `MonthlyReport`（`csv_confirmed` / `finalized`。CSV確定昇格 `confirmCsv` / 再集計 `refreshCsvConfirmed` / 最終確定昇格 `finalize` / 取引集計 `aggregateMonthlyReportTotals`（`MonthlyReportTotals`）/ 残高の凍結 `freezeBalanceSnapshot`（残高変動履歴から写し取った LINE 配信時点の値。#398）/ 不完全月判定 `isIncompleteMonthReport` を含む）
...
- Repository I/F: `AccountRepository`, `BankDepositRepository`, `MitsuiSumitomoUnpaidRepository`, `BalanceHistoryRepository`（追記 `append` は冪等 / 期間読み出し `findByOccurredAtRange` / 軸ごとの直前値 `findLatestBefore`）
- Query I/F: `AccountBalanceQuery`, `BalanceTimeSeriesQuery`（読み出し元は残高変動履歴。世帯フルオープンだが `viewerId` を受け取る）
