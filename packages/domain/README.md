...
- 値オブジェクト: `CandidateImportSource`, `AmazonOrderInfo`, `SmbcMailParseResult`, `DuplicationJudgment`, `ImportResultSummary`, `ImportJobFailureReason`, `PdfConversionFailureReason`, `ImportTargetPeriod`（`from < to` 検証述語 `importTargetPeriodIsValid`）（および `normalizeMerchantName` — OQ-23 加盟店名正規化）
