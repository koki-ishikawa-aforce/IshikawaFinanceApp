...
- 集約: `TransactionCandidate`（`normal` / `amazon_matched` / `match_timeout`）, `DailyMailImportBatch`, `StatementImportJob`（PDF/CSV ルーティング `launchImportJob`、進捗更新 `updateProcessedCount`）
