...
describe('ExpenseTypeDeletionRequest 状態遷移', () => {
  ...
  it('依頼していないコンテキストからの完了通知は記録しない', () => {
    ...
    const afterUnrequested = recordExpenseTypeRemapContextCompletion(
      requested,
      { context: 'household_analysis', affectedTransactionCount: 7, affectedLearningRuleCount: 0 },
      at,
    )
    expect(afterUnrequested.state.completedContexts).toEqual([])
    // 依頼外の通知だけでは物理削除の前提条件を満たさない
    expect(isExpenseTypeRemapFullyCompleted(afterUnrequested)).toBe(false)
    ...
    expect(completed.state.affectedTransactionCount).toBe(2)
