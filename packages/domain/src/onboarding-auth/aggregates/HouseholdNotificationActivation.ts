 ...
 * @see docs/domain/03-open-questions.md OQ-55 ①（改訂 2026-08-23 / #447、改訂 2026-08-26 / #590）
 *
 * 「世帯の通知機能を有効化し、テストメッセージが LINE に届いた（配信確定した）」という事実を
 * 世帯にひとつだけ記録する。per-user の 通知機能有効化状態 は #334 の整理どおり残るが、
 * 「もう送ったか」の判断は per-user の状態の組み合わせからの推測ではなく本記録で行う（#447）。
 *
 * 推測に頼ると「有効化の保存は成功したが送信で失敗した」回を「もう送った」と誤認し、
 * テストメッセージがその世帯へ二度と送られない。本記録は**LINE への配信が確定して初めて書く**
 * （通知配信からの `TestMessageSent` を検知して書く、#590）ため、届かなかった回（発行そのものの
 * 失敗・配信サービスの `retry_abandoned`）は次の発火の起点でやり直せる。
 *
