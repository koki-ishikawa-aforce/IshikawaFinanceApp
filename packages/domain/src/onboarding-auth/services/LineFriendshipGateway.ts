 ...
 * 取りこぼしを拾い直す唯一の経路になる。呼出し元はアプリユーザーの登録要求（新規登録の
 * 成立直後、および登録済みの冪等な再要求）で、友だち追加が未記録のときだけ照会する。
 * follow Webhook は友だち追加の瞬間にしか発生せず再送されないため、照会が失敗した回の
 * 回復も次の登録要求での再照会に依る（Webhook は回復経路にならない）。
 *
