...
- **「上限の使い切り具合を見せる」**: 経費精算管理では、上限のある経費種別について当月累計に上限額と進捗バーを併記する。上限を超えた分は自動で個人(本人) の支出へ振り替わるため、どこまで使ったかが見えないと「気づいたら個人負担になっていた」状態を招く。当初は気軽に経費フラグを付けられることを優先して「上限を意識させない（進捗バーなし・合計のみ）」としていたが、2026-08-23 の判断（#505）でこちらへ改めた
