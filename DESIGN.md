...
>
> スナップショット（変更しない）: `docs/superpowers/specs/2026-05-01-phase3.5-ux-ui-design.md`。`docs/domain/wireframes/design-tokens.md` も Phase 3.5 以前のスナップショット。`docs/superpowers/` 配下のその他の日付つき成果物（`specs/` / `plans/`）も当時の記録であり、本文書に追従させない（更新が要る場合は本文に改訂注記を残す。前例: #398）。**これらと本文書が食い違う場合は、いずれも本文書が優先する。**
...
- **「上限の使い切り具合を見せる」**: 経費精算管理では、上限あり経費種別に上限までの使い切り具合を見せ、無制限経費種別とは見せ方を変える。上限到達は色だけでなくテキストラベルの併記で示す（§6「色だけに頼らない識別」）。上限を超えた分は先着順で個人(本人) へ按分されるため、使い切り具合が見えないと気づかないまま個人負担になる。表示要素の内訳と改訂の経緯は [`docs/domain/03-open-questions.md`](./docs/domain/03-open-questions.md) OQ-16 を参照
...
- **行間（`--leading`）**: 単位なしの倍率で 1 種類（1.6）。複数行にわたる文章（補足文・注意書き・結果の説明・手順の一覧等）に使う。1 行に収まる見出し・ラベルには指定しない
...
ラベルと並ぶアイコンは `--icon-sm` を既定とし、`--icon-md` を選ぶのはラベルより意図的に目立たせる理由がある場合に限る。同じ役目のボタンが画面をまたいで別の段階を使う状態を作らない（#502）。
...
**適用範囲はアプリの画面（`packages/web`）に限る**。LINE に届くメッセージの文面（`packages/api/src/notification/`）は本ルールの対象外で、絵文字を使ってよい。LINE がメッセージを描画するため react-icons も CSS も使えず、警告や誘導の記号を絵文字以外で表現する手段が無いため（現行の文面では取込のお知らせの `📄` / `👇`、データが不完全な月の注意書きの `⚠️` がこれに当たる）。逆に、画面側でアイコン部品を使わずに済ませる理由にはならない。
...
- JSX に絵文字を表示目的で埋め込まない — react-icons のラインアイコンまたは CSS で表現する（対象はアプリの画面。LINE のメッセージ文面は対象外。§4「UI への絵文字使用は禁止」を参照）
...
- 行間を画面ごとに数値で書かない — `--leading` を使う（stylelint が直値を止める）
