...
>
> スナップショット（変更しない）: `docs/superpowers/` 配下の日付つき成果物（`specs/` と `plans/`。例: `docs/superpowers/specs/2026-05-01-phase3.5-ux-ui-design.md`）。`docs/domain/wireframes/design-tokens.md` も Phase 3.5 以前のスナップショットであり、現行ルールは本文書が優先する。
...
- **「上限の使い切り具合を見せる」**: 経費精算管理では、上限あり経費種別に当月累計・月次上限（上限金額）・進捗バーを併記し、上限到達済みはそれを明示する。無制限経費種別は上限金額と進捗バーを出さず「上限なし」と示す。上限を超えた分は先着順で個人(本人) へ按分されるため、使い切り具合が見えないと気づかないまま個人負担になる。改訂の経緯は [`docs/domain/03-open-questions.md`](./docs/domain/03-open-questions.md) OQ-16 を参照
