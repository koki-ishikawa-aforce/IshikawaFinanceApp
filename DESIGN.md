...
単独で置くアイコン（隣に大きさの連動する文字が無いもの）は、相対値の基準となる `font-size` をアイコン自身に明示する（`AppNav.module.css` の `.icon`・`app/balances/page.module.css` の `.balanceIcon` が既存例）。基準を書かずに周囲の文字サイズへ委ねると、無関係な文字サイズの変更でアイコンの大きさが動く。
