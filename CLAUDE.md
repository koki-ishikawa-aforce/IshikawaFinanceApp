...
4. DDD 観点レビュー: `/ddd-review`(PR 前に実施)— レビュー指摘は must-fix だけでなく suggestion も原則その場で修正する。見送りは例外(独立した PR が必要な別リファクタリング相当の規模、または設計判断の変更を伴いユーザーの意思決定が必要な場合)のみで、後者(設計判断)のみ Issue 化して追跡する。前者(規模)は PR 本文に見送った内容と理由を記録するに留める(詳細は各レビュー skill の SKILL.md を正とする)
