...
- OQ-38 ⚠️(部分) 三井住友銀行（SMBC ダイレクト）の月別明細 URL パターン実調査（取引取込スコープ）→ **2026-07-22 のクローズはスコープが異なる**。03-open-questions.md の OQ-38 はこの時点で「SMBC 通知**メール**のフォーマット実調査」に読み替えられており、解決したのは通知メール 3 種のフォーマット（メール取込の対象はカード利用通知のみ）。本項が元々問うていた **CSV/PDF を取得しに行く明細ページの URL パターンは未解決のまま**で、Phase 3.5 spec §10.2 の三井住友銀行の URL テンプレートは `https://direct.smbc.co.jp/...` の placeholder が残っている（同 §10.2 の三井住友カード側は確定済み）
- OQ-39 ✅ LINE Flex Message のサイズ制限内に Honey/Darling 別リンクが収まるか検証（通知配信スコープ）→ **収まる**（2026-07-24）。4 リンク同梱の CSV取込リマインダーで 1,895 B（50KB 上限の 3.7%）
