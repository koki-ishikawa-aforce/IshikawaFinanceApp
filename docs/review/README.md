...
4. **人間にしか判断できないものは人間に残す**
   着手承認(Issue への `ready-to-implement` 付与)、本番受入テストの合否、実機 LIFF での体験、リスクの受容。無人モードはこれらを代行しない。
   **マージ可否はここに含まない。** CI・レビュースキル・機械的なマージゲート(`.claude/skills/issue-work/SKILL.md`)がすべて green なら無人モードがマージする。人間の承認は「着手前のラベル付け」に前倒しで集約されている(`CLAUDE.md` の開発フロー節)。個別 PR を止めたいときは `needs-decision` を付ける。
...
- 見送りは例外のみ: **(a) 独立した PR が必要な別リファクタリング相当の規模**、または **(b) 以後の実装の前提を変える設計判断でユーザーの意思決定が必要**(基準の詳細な定義は各レビュー skill の SKILL.md を正とする)
- 見送る場合も黙って放置しない。**Issue 化するのは (b) のみ**(タイトル先頭 `[判断待ち]` + `needs-decision` を付けて `/decide` に接続する)。(a) は Issue を起票せず、見送った内容と理由を PR 本文のレビュー結果節に記録する(同じ指摘が複数の PR で繰り返し見送られた場合に限り Issue 化してよい)
...
無人モードが自分でマージするようになったため、この設定は **`verify` が green でない変更が `main` に入らないことを GitHub 側で保証する最後のガード**でもある。スキル側のマージゲートが誤って緩んでも、required check がマージを拒否する。以下の値は変更しない。
...
無人モードのマージゲートは、この設定の代わりに **同一 fire で2件目以降をマージするときだけ追従を必須にする**(`main` を取り込んで CI を再度 green にしてからマージする)。連続マージという最も衝突しやすい局面を押さえつつ、通常の1件マージでは CI 1周ぶんの待ちを増やさないための折衷。さらに、マージ後に `main` が赤くなったらその fire は以降のマージを止めるため、巻き添えは1件で止まる。
