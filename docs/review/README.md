...
- 見送りは例外のみ:(a) 独立した PR が必要な別リファクタリング相当の規模、または (b) 以後の実装の前提を変える設計判断でユーザーの意思決定が必要な場合。Issue 化して追跡するのは (b) のみ(`needs-decision` を付けて `/decide` に接続する)。(a) は PR 本文のレビュー結果節に見送った内容と理由を記録するに留める(基準の詳細は各レビュー skill の SKILL.md を正とする)
