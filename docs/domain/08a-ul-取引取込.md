...
// 変換成功は生成 CSV を永続化せず、抽出した明細行を直接取引候補の生成材料とする（PR #55）。
data 変換成功 = PDF変換ジョブID AND List<変換明細行> AND 変換完了日時
data 変換明細行 = 発生日時 AND 加盟店名 AND 金額 AND ページ番号
...
data メール取得失敗 = OAuth失効状態 AND 検知日時 AND 取得失敗詳細 AND 再試行可否
data OAuth失効状態 = OAuth失効検知 OR その他取得失敗
data 取得失敗詳細 = 文字列
data 再試行可否 = 一時障害 OR 恒久的
// 事後: OAuth失効検知 は その他取得失敗 と区別する（通信断・API 障害を根拠に再認可を求めない）
// 事後: 再試行可否 は「同じ取得をやり直せば結果が変わりうるか」。通信断・タイムアウト・
//       レート制限・5xx は 一時障害、設定不備・応答構造の不正は 恒久的
// 事後: 取得失敗詳細 はログ・通知に載る前提の短い文言で、シークレット・PII・メール本文を含めない
// 事後: 部分成功は表現しない（取得できた分だけを返すと、取りこぼしを完了として記録してしまう）

behavior SMBC通知メール本文をパースする = SMBC通知メール本文 AND ユーザーID AND 検知日時 -> SMBC通知パース結果 OR パース失敗
// 事前: ユーザーID・検知日時は本文から決まらないため呼出し側（日次メール取込バッチ）が渡す
...
behavior PDFから取引候補を生成する = 変換成功 AND 取込ジョブID -> List<通常取引候補> AND CSV取込完了イベント OR 取込失敗ジョブ
// 事前: PDF→CSV 変換が成功し、変換明細行を取得済み（生成 CSV は経由しない）
// 事後: 取込ソース = PDF由来（アップロードファイルID AND ページ番号 AND PDF変換ジョブID）
...
// 事後: 変換成功なら生成 CSV を永続化せず、抽出した変換明細行を直接「PDFから取引候補を生成する」に渡す（PR #55）
...
// 発行: 前回の取込が完了しないまま残したバッチを、次の起動が引き継いで再開したとき（#414）。
//       取得失敗で終わったバッチは失敗（終端）として閉じるため、再認可後の取込は引き継ぎではなく
//       新しいバッチの起動になる（再認可を起点とする通知は 08f §2「Gmail OAuth を再認可する」側）
```
