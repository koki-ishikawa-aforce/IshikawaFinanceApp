...
> 改訂（2026-08-23・判断セッション / #447、OQ-55 ①）: 世帯レベルの集約 **世帯通知有効化記録** を新設（§1）。`behavior 通知機能を有効化する` の二重発行を防ぐ根拠を、per-user の `有効化済み` の組み合わせからの推測ではなくこの記録に変えた（§2）。記録は通知機能有効化イベントの発行が成功して初めて書くため、発行に失敗した回は次の発火の起点でやり直せる。従来はその回が「もう送った」と誤認され、テストメッセージがその世帯へ二度と送られなかった。
> 改訂（2026-08-26・判断セッション / #590、OQ-55 ①）: 世帯通知有効化記録を書くタイミングを、通知機能有効化イベントの発行成功から**通知配信での LINE 配信確定**までさらに一歩遅らせた。#447 が閉じたのは「発行そのものが失敗した回」の穴で、発行後に LINE への送信が最終的に失敗した回（配信サービスの `retry_abandoned`）はなお記録が残ってしまい回復できなかった。新設した `behavior 世帯通知有効化を記録する`（§2）が通知配信からの `テスト送信イベント`（配信確定。実送信・冪等スキップいずれも含む）を受けて記録する。`behavior 通知機能を有効化する` はテストメッセージ送信の依頼までを担い、記録そのものは行わない。
...
> 改訂（2026-08-26・判断セッション / #684、docs-drift）: `data Gmail_OAuth_トークン` に `// aggregate root` マーカーを追記した。実装（`packages/domain/src/onboarding-auth/aggregates/GmailOAuthToken.ts`）は当初から専用 Repository を持つ独立した集約であり、09-aggregates §1 の集約行（#14d）とあわせて docs 側を実態に揃えた（定義・振る舞いに変更はない）。

> 改訂（2026-08-28・判断セッション / #651、決定 A-1）: 世帯外の相手からのアクセス拒否を記録する集約 **アクセス拒否カウンタ** を新設した（§1 §2）。従来、入口の許可リスト照合ガード（#533）が断った回はサーバログにのみ残り DB には一切保存されていなかった。同じ相手からの拒否は 1 件へ集約し（LINE_userID ごとに累計回数・最終発生日時のみを保持）、個々の発生時刻の履歴は残さない。09-aggregates §1 の集約行（#14e）に追記した。
...
// --- 世帯通知有効化記録: 集約ルート（世帯レベル） ---
// 「世帯としてテストメッセージの送信を依頼した」も世帯にひとつの事実であり、per-user の
// 通知機能有効化状態 の組み合わせから推測しない（改訂 2026-08-23・判断セッション / #447）

// aggregate root
data 世帯通知有効化記録 = 世帯通知有効化状態

data 世帯通知有効化状態 = 世帯未有効化 OR 世帯有効化済み
data 世帯有効化済み = 有効化日時
// 「テストメッセージをもう送ったか」を判断する唯一の根拠。通知配信からの テスト送信イベント
// （LINE への配信確定。実送信・冪等スキップいずれも含む）を受けて初めて記録するため、
// 発行そのものの失敗・配信の到達失敗（retry_abandoned）はいずれも記録が残らず、
// 次の発火の起点でやり直せる（#590）。
// per-user の状態から推測していた頃は、有効化の保存に成功して発行で失敗した回が「もう送った」と
// 誤認され、テストメッセージがその世帯へ二度と送られなかった（#447）。通知機能有効化イベントの
// 発行成功だけを根拠にしていた頃は、発行後に配信が最終失敗した回がなお回復できなかった（#590）。
...
// --- アクセス拒否カウンタ: 集約ルート ---
// 見知らぬ相手からのアクセス拒否を LINE_userID ごとに集約する（Issue #651 決定 A-1）。
// 同じ相手からの拒否は 1 件へ更新し続け、個々の発生時刻の履歴は残さない
// （既存の連続失敗カウンタ〔08g〕と同じ考え方）。

// aggregate root
data アクセス拒否カウンタ = LINE_userID AND 拒否累計回数 AND 最終拒否日時
...
//       やり直しても直らないため再送に回さない。回復は招待のやり直しによる
//       （自己申告 API は #298 で廃止済み）
...
// 事前: 両者ともに友達追加済み・世帯が共通トークルーム参加済み・世帯未有効化
// 事後: 通知配信にテストメッセージ送信を依頼
// 共通トークルーム参加状態は前提条件の検証にのみ使う。有効化済み は共通トークルームID を
// 保持せず、イベントへ載せる配信先は世帯レベルの 共通トークルーム から取る（#334）
// 世帯有効化済み はここでは記録しない（#590。記録は下記 behavior 世帯通知有効化を記録する が
// 担う）。事前条件の 世帯未有効化 が通知機能有効化イベントの二重発行を防ぐ根拠であり、
// per-user の 有効化済み は根拠にしない（保存に成功して発行に失敗した回を「もう送った」と
// 誤認しないため）。同時発火（起点は 4 つある）に対する一意性の最終保証は永続化層の singleton
// 制約が担う。世帯未有効化 のまま前提が揃うたびに再発行されうるが、LINE への実送信は通知配信の
// 冪等性キーで 1 回に絞られるため実害にならない（08g 参照）

behavior 世帯通知有効化を記録する = テスト送信イベント -> 世帯有効化済み
// 事前: 通知配信から テスト送信イベント（LINE への配信確定。実送信・冪等スキップいずれも含む）
//       を受信
// 事後: 世帯有効化済み を記録する（#590。LINE への配信が確定して初めて「もう送った」とみなす）
// 事後: 記録が既に世帯有効化済みなら上書きしない（冪等。同一の配信確定に対して
//       テスト送信イベント が複数回発行されうるため）
...
behavior アクセス拒否を記録する = アクセス拒否イベント -> アクセス拒否カウンタ
// 事前: アクセス拒否イベント（許可リスト不一致による拒否。画面経由の登録要求・API 直接呼出しの
//       入口ガード（#533）いずれも発行元になる）を受信
// 事後: 対象 LINE_userID のカウンタが無ければ1件目として作り、あれば拒否累計回数を+1し
//       最終拒否日時を更新する（決定 A-1。個々の発生時刻の履歴は残さない）
```
...
| 通知配信 | 顧客-供給者（下流: テスト送信／OAuth 失効通知／LINE userID・共通トークルームID を供給。上流: テスト送信イベント（配信確定）を供給、#590） | `behavior 通知機能を有効化する`／`behavior Gmail OAuth トークンの失効を検知する` の出力イベント／`behavior 世帯通知有効化を記録する`（テスト送信イベントの受信側） |
