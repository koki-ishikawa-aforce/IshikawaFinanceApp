...
data 世帯有効化済み = 有効化日時
// 「テストメッセージをもう送ったか」を判断する唯一の根拠。通知機能有効化イベントの発行が
// 成功して初めて記録するため、発行に失敗した回は次の発火の起点でやり直せる。
// per-user の状態から推測していた頃は、有効化の保存に成功して発行で失敗した回が「もう送った」と
// 誤認され、テストメッセージがその世帯へ二度と送られなかった（#447）。
// 発行のあとに起きる失敗（通知配信が LINE への送信を諦めた回）は本記録では回復しない
// — 記録の確定を配信の到達まで遅らせるかは #590 の判断待ち
...
behavior 通知機能を有効化する = 運用開始済みユーザー(Honey) AND 運用開始済みユーザー(Darling) AND 共通トークルーム参加状態 -> 有効化済み AND 通知機能有効化イベント AND 世帯有効化済み
...
// 世帯有効化済み は 通知機能有効化イベント の発行が成功して初めて記録する（#447）。
// 事前条件の 世帯未有効化 が二重発行を防ぐ根拠であり、per-user の 有効化済み は根拠にしない
// （保存に成功して発行に失敗した回を「もう送った」と誤認しないため）。
// 同時発火（起点は 4 つある）に対する一意性の最終保証は永続化層の singleton 制約が担う
