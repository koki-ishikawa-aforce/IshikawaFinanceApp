...
> 改訂（2026-08-23・判断セッション / #447、OQ-55 ①）: 世帯レベルの集約 **世帯通知有効化記録** を新設（§1）。`behavior 通知機能を有効化する` の二重発行を防ぐ根拠を、per-user の `有効化済み` の組み合わせからの推測ではなくこの記録に変えた（§2）。記録は送信の依頼が成功して初めて書くため、依頼に失敗した回は次の発火の起点でやり直せる。従来はその回が「もう送った」と誤認され、テストメッセージがその世帯へ二度と送られなかった。
...
data 世帯有効化済み = 有効化日時
// 「テストメッセージをもう送ったか」を判断する唯一の根拠。送信の依頼（通知機能有効化イベントの
// 発行）が成功して初めて記録するため、依頼に失敗した回は次の発火の起点でやり直せる。
// per-user の状態から推測していた頃は、有効化の保存に成功して依頼で失敗した回が「もう送った」と
...
// 事前条件の 世帯未有効化 がイベント二重発行を防ぐ唯一の根拠であり、per-user の 有効化済み は
// 根拠にしない（保存に成功して発行に失敗した回を「もう送った」と誤認しないため）
