...
| 2 | **着手承認** | open Issue | **人間のみ**(`/backlog-ready` は判定補助) | `ready-to-implement` ラベル | 6基準(リポジトリ内で完結 / 受け入れ条件が検証可能 / 依存解決済み / 設計判断なし / 1 PR 粒度 / 新規画面・フローはレイアウト仕様あり)をすべて満たす | `.claude/skills/backlog-ready/SKILL.md` |
...
| レビュー suggestion の見送り | ユーザーと相談し、設計判断(基準 (b))のみ Issue 化。規模理由(基準 (a))は PR 本文に記録 | 設計判断(基準 (b))のみ `[判断待ち]` Issue を起票して `needs-decision`。規模理由(基準 (a))は PR 本文に記録し起票しない |
