...
| KA10 | Software Engineering Process(プロセス) | 本 docs 群自体(工程定義)、`/retro` → `/decide`(プロセス改善ループ)、`05-criteria.md`(プロセス測定) | ✅ | fire の実行結果を tracking Issue へ1行 JSON で記録する構造化レポート(#740)を導入済みで、`/retro` の自由テキスト発掘依存は解消した(`05-criteria.md` 基準6は○に改訂済み、#787) |
