...
| イベント | bot の動作 | メール通知 |
| --- | --- | --- |
| Issue に `needs-decision` が付いた | あなたを assignee に追加 + @メンションコメント | **あり**(人間の判断が必要) |
| PR が作成された(Draft・通常を問わない) | あなたを assignee に追加 | **あり**(マージ判断 Issue が作られなかった場合の保険も兼ねる) |
| PR がマージ/クローズされた | 対応するマージ判断 Issue の assignee を外してクローズ(コメントなし) | **なし**(自動処理。クローズ理由と Issue タイムラインで追跡可能) |
