...
`docs/domain`(集約定義・ユビキタス言語・公開 API 一覧)と `packages/domain` のコードの乖離、および `docs/workflow/03-agent-runtime.md`(実行基盤の一覧)と `.claude` / `.github` の実体の乖離を、Claude Code の Routine(定期実行・fire ごとに fresh session)で週次に検知し、乖離を Issue として起票する仕組み。手順の本体は `.claude/skills/docs-drift/SKILL.md` にあり、このドキュメントはその運用(Routine 設定・スコープ)を定める。
...
Routine(週次 fire・fresh session): /docs-drift を実行
  ...
  ├─ 突合 D: docs/workflow/03-agent-runtime.md の実行基盤一覧 ↔ .claude / .github の実体
...
| 乖離検知 Routine(`/docs-drift`) | docs とコード・実行基盤の乖離を定期検知 | 乖離報告 Issue(ready-to-implement または needs-decision) | docs/domain と packages/domain/src、docs/workflow と .claude / .github |
...
`/docs-drift` は以下の 4 つの突合を行う(詳細は `.claude/skills/docs-drift/SKILL.md` 手順 2〜5):
...
- **突合 D: 実行基盤の一覧 ↔ 実体** — `docs/workflow/03-agent-runtime.md` の skill / サブエージェント / hooks / Routine / GitHub Actions の一覧(名前・件数・cron・ジョブ名)と、`.claude/` 配下・`.github/workflows/`・`docs/automation/*.md` の記録を照合する。ラベル表(`docs/workflow/02-labels.md`)と各 SKILL.md 埋め込みの `gh label create` の値の不一致も検出する

## Routine のセットアップ

[claude.ai](https://claude.ai) の Claude Code → Routines から作成する(Routine はクラウド側で動くため、手元のセッションや PC の状態に依存しない)。

**登録状況**: 2026-08-24 に MCP の `create_trigger` 経由で登録済み(trigger ID `trig_01FDcNzRrGukacg1hJpeGZtB`)。
...
- **Trigger**: Schedule、週次(例: 毎週水曜の朝)。現在は UTC で `0 0 * * 3`(= 水曜 09:00 JST)。バックログ Routine(2時間おき)や振り返り Routine(毎週月曜)とずらすのが望ましい — 振り返りが拾う失敗データとドキュメント乖離は独立した関心事のため、曜日を分散させて負荷を分ける
...
| 突合対象 | A(公開 API)・B(集約定義)・C(UL 用語)・D(実行基盤一覧) | SKILL.md 手順 1 |
| ready-to-implement の自動判定 | docs/コードのどちらが正かが自明な場合 | SKILL.md 手順 6 |

## 止め方・トラブル時

- **一時停止**: Routine を無効化する(claude.ai の Routines 画面)。実行中の fire には影響しない
- **乖離報告が過剰 / 的外れ**: `ready-to-implement` が付いた乖離は自動で実装されるため、不要な乖離報告が `ready-to-implement` で起票されていないか確認する。的外れな報告が続く場合は SKILL.md の突合ロジック(手順 2〜5)を見直す
- **乖離報告 Issue が溜まる**: `needs-decision` の分は `/decide` で方針を判断する。`ready-to-implement` の分はバックログ Routine が順次消化する
- **同じ乖離が毎週起票される**: SKILL.md 手順 6 の重複チェックは open と closed の両方を検索し、not planned でクローズされた Issue は「意図的な差異」として再起票しない。それでも重複が起きる場合は、既存 Issue のタイトルやキーワードが異なるために重複を検出できていない可能性がある。意図的な差異として再起票を止めたい場合は、対象の乖離報告 Issue を not planned で閉じる
